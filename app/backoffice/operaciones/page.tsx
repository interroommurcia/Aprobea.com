...
export default function OperacionesPage() {
  ...
  async function buscarCatastro(rcParam?: string) {
    ...
    try {
      // Catastro REST API: Consulta_DNPRC and Consulta_CPMRC both require a 14-char RC.
      // 20-char RCs (with localizador) must be truncated to the 14-char parcel base.
      const rc14 = rc.length > 14 ? rc.slice(0, 14) : rc
      const detailUrl = `https://ovc.catastro.meh.es/OVCServWeb/OVCWcfCallejero/COVCCallejero.svc/rest/Consulta_DNPRC?Provincia=&Municipio=&RC=${encodeURIComponent(rc14)}`
      const coordUrl  = `https://ovc.catastro.meh.es/ovcservweb/OVCSWLocalizacionRC/ovccoordenadas.asmx/Consulta_CPMRC?Provincia=&Municipio=&SRS=EPSG:4326&RC=${encodeURIComponent(rc14)}`
